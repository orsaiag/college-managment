...
public class Model {
	...
	public static Object getObjectFromFile(int type, String id, String file) {
		...
		try {
			...
			case 5: {
				Secratery object = new Secratery(id, info[1], info[2], info[3], info[4], info[5],
						(new Integer(info[6])));
				obj1 = (Object) object;
				return (Object) object;
			}
			case 6: {
				Principal object = new Principal(id, info[1], info[2], info[3], info[4], info[5],
						(new Integer(info[6])));
				obj1 = (Object) object;
				return (Object) object;
			}
	...
	public String showSpecipicLinesFromFile(String id, String filename) throws IOException {
		String print="";
		File file = new File(filename);
		Scanner in = null;
		try {
	...
	public String showGrades(String id, String filename) throws IOException {
		String print="";
		/*
		 * there are no 100, only 99 and below! exam is 80% from the grade and quiz is
		 * 20% from grade: 1-EXAM 2-QUIZ
		 * 
		 * the course are: 1-infi1 2-algebra 3-CS 4-Java
		 */
		//System.out.println("Grades and overall average:");
		print+="Grades and overall average: \n";
		File file = new File(filename);
			...
			while (in.hasNext()) {
				...
				if (line.contains(id)) {
					...
					case '1': {
						...
						print+="Exam- " + number1 + number2+"\n";
						break;
					}
					case '2': {
						sum += (number1 * 10 + number2) * 0.2;
						print+="Quiz- " + number1 + number2+"\n";
						break;
					}
					}
				}
			}
			print+="\nOver all average: " + sum+"\n";
		} catch (FileNotFoundException e) {
			print+="File not found\n";
			e.printStackTrace();
		}
	...
	@SuppressWarnings({ "resource" })
	public String showStudentPerLecturerOrTutor(String id, String filename) throws FileNotFoundException {
		/*
		 * the course are: 1-infi1 2-algebra 3-CS 4-Java
		 */
		String print = "";
	...
	public static String modifyFile(String filePath, String id, String oldString, String newString) {
		String print="";
			...
			while (line != null) {
				...
				if (line.contains(id)) {
					if (line.contains("," + oldString))
						oldContent = oldContent + line.replace("," + oldString, "," + newString)
					...
					while (lineObjects != null) {
						if (lineObjects.contains(id)) {
							...
							oldContent = oldContent
									+ lineObjects.replaceAll(lineObjects, line.substring(2, line.length()))
									+ System.lineSeparator();
	...
	@SuppressWarnings({ "resource" })
	public String showGradesOfAllStudents(String filename) throws IOException {
		String print="";
	...
	@SuppressWarnings("resource")
	public boolean checkIfObjectExists(String id, String filename) throws FileNotFoundException {
	...
	public boolean addNewStuentOrWorker(int type, String id, String name, String email, String password, String address,
			String dateOfBirth, int number) throws IOException {
		if (id.length() == 9) {
			if (dateOfBirth.indexOf('/', 2) == 2 && dateOfBirth.indexOf('/', 5) == 5) {
				if (checkIfObjectExists(id, "User.txt") == false) {
					...
					case 1: {
						...
						u.name(name).email(email).address(address).email(email).number(number).build();
						break;
					...
					case 5: {
						...
						Secratery s = Secratery.getSecratery(id, name, email, password, address, dateOfBirth, number);
						if (s == null)
							u = null;
						break;
					}
					case 6: {
						...
						Principal p = Principal.getPrincipal(id, name, email, password, address, dateOfBirth, number);
						if (p == null)
							u = null;
						break;
					}
				...
				System.out.println("Wrong format of date of birth");
		} else
	...
	public boolean addGrade(String id, String course, String type, String grade) throws IOException {
		String print="";
		if((new Integer(grade))<0 || (new Integer(grade))>100)
		{
			print+="Wrong grade\n";
			//return print;
			return false;
		...
		if (student) {
			if (type.equals("1"))
			{
				//return new Exam(id, course, grade);
				new Exam(id, course, grade);
				return true;
			}
			else
			{
				//return new Quiz(id, course, grade);
